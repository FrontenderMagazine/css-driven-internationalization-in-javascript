...
задача локализации. Несмотря на то, что в [текущем стандарте][1] представлен ряд 
...
В этой статье я опишу свой подход к решению задачи локализации с помощью 
...
Изначально, решение должно было базироваться на наборе **API локализации для 
...
метод `$.tools.validator.localize`, вот так:
...
проекте испольуются вызовы AJAX, те же шаги нужно будет предпринять также и для 
...
более надёжен чем вариант с атрибутом, так как он работает должным образом даже 
когда атрибут `:lang` для элемента не прописан.

Пример выше показывает как можно изменить **представление контента в зависимости 
от текущего языка с помощью CSS**. Это важно, поскольку даёт нам возможность 
**использовать в CSS логику для изменения языка.**

Пример с символом кавычек неплох, но он сработает только в очень ограниченном 
количестве случаев и не может быть использован в типичных сценариях — строки 
повседневного использования в разных языках слишком сильно отличаются. Нам нужен 
трюк, который позволит изменять содержимое элемента целиком.
...
Обратите внимание, что в Internet Explorer 8 существует проблема в реализации 
спецификации, которая состоит в том что он не поддерживает синтаксис двойного 
двоеточия при определении псевдоэлементов. В Internet Explorer 9 эта ошибка 
исправлена, однако если вам нужна поддержка 8 версии, для всех псевдоэлементов 
используйте только синтаксис с одним двоеточием. 
...
мультиязычных строк, которые вообще-то должны быть содержимым веб-страницы. Не 
очень хорошо.
...
В better-dom для поддержки локализации с помощью CSS предусмотрены две функции: 
...
С помощью `DOM.importStrings` можно также переопределить существующие строки на 
...
шаблонный код и позволяют использовать локализацию на основе CSS на наших 
веб-страницах без затруднений.
...
Следовательно, этот подход достаточно не громоздкий.
...
концепцию пригодной для использования.

Убедитесь что используете её в тех частях страниц, которые не являются жизненно 
важными для SEO, так как ботам поисковиков будет трудно правильно 
...
решить я спрятал исходный элемент и добавил дополнительный элемент `<span>` в 
...
**Обновлено (24.06.2014):** В комментариях к статье несколько людей отметили, 
...
Следовательно, я улучшил концепцию с учётом этих моментов и хочу пригласить вас 
[посмотреть демо][15]. Оно пока не включено в better-dom API, однако будет 
...
Использование внутренних `<span>`ов вместо псевдоэлементов также решает проблему 
...
локализации в JavaScript. До того времени нам, фронтенд разработчикам, придётся 
заново придумывать колесо или же приспосабливать колёса друг друга под свои 
потребности.

В процессе разработки этого подхода к локализации на основе CSS меня вдохновили 
идеи, которые содержатся в самой спецификации CSS2. Возможно её автора также 
